...
const AnimatedBackground = () => {
  ...
  const handleMouseHover = (e: SplineEvent) => {
    if (!splineApp || activeSectionRef.current === "skills" || selectedSkillRef.current?.name === e.target.name) return;
  ...
  const handleSplineInteractions = () => {
    ...
    splineApp.addEventListener("keyUp", () => {
      if (!splineApp || activeSectionRef.current === "skills" || isInputFocused()) return;
      playReleaseSound();
      splineApp.setVariable("heading", "");
      splineApp.setVariable("desc", "");
    });
  ...
  useEffect(() => {
    ...
    const manageAnimations = async () => {
      // Reset text if entering skills (since it's now non-interactive) or other sections
